...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        # Backprop through first (affine + ReLU) layer
...
class FullyConnectedNet(object):
    ...
    def __init__(
        ...
        ############################################################################
        # Build list of layer dimensions: input -> hidden(s) -> output
        ...
        # If using batchnorm or layernorm, initialize gamma and beta for hidden layers
        if self.normalization in ['batchnorm', 'layernorm']:
            for i in range(self.num_layers - 1):
                gi = 'gamma%d' % (i + 1)
                bi = 'beta%d' % (i + 1)
                self.params[gi] = np.ones(layer_dims[i + 1])
                self.params[bi] = np.zeros(layer_dims[i + 1])
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
    ...
    def loss(self, X, y=None):
        ...
        # layer, etc.                                                              #
        ############################################################################
        # We'll perform (affine - [norm] - relu - [dropout]) for L-1 layers,
        # then an affine for the final layer to produce scores.
        caches = []
        out = X

        # Forward through first L-1 layers
        for i in range(self.num_layers - 1):
            idx = i + 1
            W = self.params['W%d' % idx]
            b = self.params['b%d' % idx]

            # Affine forward (or affine+relu convenience if no normalization)
            if self.normalization is None:
                ...
                a, norm_cache = (
                    batchnorm_forward(a, gamma, beta, self.bn_params[i])
                    if self.normalization == 'batchnorm'
                    else layernorm_forward(a, gamma, beta, self.bn_params[i])
                )
            ...
            if self.use_dropout:
                ...
                # store dropout cache together with layer cache
        ...
        # Save caches for backward pass in local variable
        self._fc_caches = caches
        ...
        # of 0.5 to simplify the expression for the gradient.                      #
        ############################################################################
        # Compute data loss and initial gradient from softmax
        data_loss, dscores = softmax_loss(scores, y)

        # Regularization loss: sum over all weight matrices
        reg_loss = 0.0
        ...
        # Backprop into final affine layer
        grads = {}
        caches = self._fc_caches

        # final cache is last element
        ...
        for i in range(self.num_layers - 2, -1, -1):
            ...
            # If dropout was used, cache was stored as (layer_cache, drop_cache)
            if self.use_dropout:
                layer_cache, drop_cache = cache
                dout = dropout_backward(dout, drop_cache)
                cache = layer_cache

            # cache is either from affine_relu_forward (fc_cache, relu_cache)
            # or from (fc_cache, norm_cache, relu_cache) when normalization used
            if self.normalization is None:
                dx, dW, db = affine_relu_backward(dout, cache)
                dW += self.reg * self.params['W%d' % layer_idx]
                ...
                if self.normalization == 'batchnorm':
                    dan, dgamma, dbeta = batchnorm_backward(da, norm_cache)
                else:
                    dan, dgamma, dbeta = layernorm_backward(da, norm_cache)
